# 콘셉트 아트 갤러리

> 아래 이미지는 세계관과 실루엣을 확인하기 위한 1차 시안이다. 이름, 성별, 세부 능력이 아직 결정되지 않은 인물의 이미지는 정본 디자인이 아니며, 해당 결정 후 수정할 수 있다.

## 1. 주인공과 늑대

![주인공과 큰 늑대](01-protagonist-and-wolf.png)

- 피와 얼음이 나뉜 이마의 소울스톤, 등에 멘 쌍검, 자연의 큰 늑대를 표현했다.

## 2. 적응 진화한 첫째 형

![적응 진화한 첫째 형](02-older-brother-adaptive-evolution.png)

- 인간 장군의 외형과 한쪽에만 발현된 적응 진화, 실험 병기라는 배경을 표현했다.

## 3. 1대 원수와 빙관의 군주

![1대 원수와 빙관의 군주](03-first-marshal-lord-of-frozen-crown.png)

- 거대한 얼음 마법보다 절제된 얼음 보조와 완성된 검술을 중심에 뒀다.

## 4. 비극 이전의 가족

![비극 이전의 가족](04-family-before-tragedy.png)

- 목걸이 속 가족사진의 기반 시안. 부모, 두 아들, 어머니와 함께 자란 늑대를 담았다.

## 5. 반인반수의 지도자와 2인자

![늑대인간 지도자와 사자인간 2인자](05-beastfolk-leaders.png)

- 회백색 늑대인간이 유일한 최종 지도자이고 사자인간이 존중받는 공식 2인자라는 관계를 표현했다.
- 사자인간의 너클은 여러 선조의 돌을 품지만 한 순간에는 하나만 빛난다.

## 6. 트롤 대장

![수정동굴의 트롤 대장](06-troll-leader.png)

- 단순한 힘캐가 아니라 수정 광산과 선조의 기술을 지키는 지도자. 충격이 쌓일수록 무거워지는 수정 둔기를 들고 있다.

## 7. 골렘 대장

![자연 생명인 골렘 대장](07-golem-leader.png)

- 기계나 소환물이 아닌 자연 생명이며, 몸을 다리와 방벽으로 바꾸어 피난민을 돕는 역할을 표현했다.

## 8. 거미인간과 곤충연합

![거미인간 대장과 곤충연합](08-spiderfolk-insect-alliance.png)

- 거미, 딱정벌레, 벌, 사마귀, 나방 공동체가 독립성을 유지한 채 통신망으로 연결된 연합이다.

## 9. 오크 검성 호위무사

![오크 검성 호위무사](09-orc-sword-saint.png)

- 광전사가 아니라 정확한 거리와 최소 동작으로 난민과 주인공을 지키는 검객이다. 고유 능력은 아직 미정이다.

## 10. 반인반용 동료

![반인반용 동료](10-half-dragon-companion.png)

- 자신의 혼혈성을 긍정하는 인물의 초기 시안. 성별과 구체 능력은 아직 정본이 아니다.

## 11. 버림받은 인간 세 사람

![붉은 번개 주술사, 단검 사용자, 도끼 전사](11-abandoned-human-trio.png)

- 장로회 실험 생존자, 학살 명령을 거부한 정보 요원, 괴물 아이를 지킨 탈영병을 한 팀으로 표현했다.

## 12. 인간 연합 지휘부

![3대 원수와 네 군 대장](12-human-alliance-command.png)

- 중앙의 회색 염동력 3대 원수와 모래·바람·강철·빛의 네 군 대장 시안이다.

## 13. 흑영대

![그림자 특수부대 흑영대](13-shadow-unit.png)

- 암살보다 수용소 개방과 구조를 수행하는 인간·괴물 혼성 특수부대다. 리더의 출신은 아직 미정이다.

## 14. 어인족과 하피족

![지역 동맹 어인족과 하피족](14-merfolk-and-harpy-allies.png)

- 여행조가 아니라 바다와 하늘에서 각자의 고향을 지키며 최종전의 구조를 돕는 지역 동맹이다.

## 15. 살아난 성과 최종 구조전

![죽으면서 무너지는 성의 신생명](15-living-castle-final-battle.png)

- 골렘이 아닌 새로운 생명. 중세 성 전체가 신경과 장기를 가진 육체가 되고, 생명이 죽으면서 성이 자연 붕괴한다.
- 마지막 전투의 중심을 처치가 아니라 인간과 괴물이 함께 수행하는 민간인 구조에 뒀다.

## 제작 기록

- 공통 방향: 오리지널 다크 판타지 웹툰·애니메이션 혼합풍, 회화적 셀 셰이딩, 현실적인 해부와 재질, 영화적 조명.
- 이미지에는 로고, 워터마크, 작품명을 넣지 않았다.
- 재현과 수정용 요약 프롬프트는 [PROMPTS.md](PROMPTS.md)에 기록했다.
